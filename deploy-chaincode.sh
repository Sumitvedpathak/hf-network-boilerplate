...
VERSION="1"
CC_SRC_PATH="./artifacts/src/fabcar/go"
CC_NAME="fabcar"
SRC_PATH="./artifacts/src/fabcar/"

packageChaincode() {
    setGlobalsForPeer0Org1
    peer lifecycle chaincode package $SRC_PATH${CC_NAME}.tar.gz \
...
installChaincode() {
    setGlobalsForPeer0Org1
    peer lifecycle chaincode install $SRC_PATH${CC_NAME}.tar.gz
...
queryInstalled() {
    ...
    peer lifecycle chaincode queryinstalled >&./artifacts/src/log.txt
    cat ./artifacts/src/log.txt
